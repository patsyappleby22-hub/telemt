...
import {
  ...
  Trash2, RotateCcw, Key, Power, PowerOff, Eye, ChevronDown,
  Copy, Check, X, AlertTriangle, Shield
} from 'lucide-react'
import { api, formatBytes, formatBps } from '../api'
...
function CreateUserModal({ onClose, onCreated }) {
  const [form, setForm] = useState({
    username: '', secret: '', user_ad_tag: '',
    max_tcp_conns: '', data_quota_bytes: '',
    rate_limit_up_bps: '', rate_limit_down_bps: '',
    max_unique_ips: '', expiration_rfc3339: '', enabled: true
  })
        ...
        <div className="space-y-4">
          <div className="p-4 bg-green-900/20 border border-green-700/40 rounded-xl">
            ...
            <div className="flex items-center gap-2">
              <code className="flex-1 font-mono text-sm bg-dark-900 px-3 py-2 rounded-lg text-yellow-300 border border-dark-600 break-all">
                {result.secret}
              </code>
    ...
    <Modal title="Создать пользователя" onClose={onClose} size="lg">
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-xs text-gray-500 mb-1.5 font-medium">Имя пользователя *</label>
            <input
              className="input"
              placeholder="alice"
              value={form.username}
          ...
          <div>
            <label className="block text-xs text-gray-500 mb-1.5 font-medium">Секрет (32 hex, пусто = авто)</label>
            <input className="input" placeholder="авто-генерация" value={form.secret} onChange={e => set('secret', e.target.value)} />
          </div>
          <div>
            <label className="block text-xs text-gray-500 mb-1.5 font-medium">Max TCP соединений</label>
            <input className="input" type="number" placeholder="не ограничено" value={form.max_tcp_conns} onChange={e => set('max_tcp_conns', e.target.value)} />
          </div>
          <div>
            <label className="block text-xs text-gray-500 mb-1.5 font-medium">Max уникальных IP</label>
            <input className="input" type="number" placeholder="не ограничено" value={form.max_unique_ips} onChange={e => set('max_unique_ips', e.target.value)} />
          </div>
          <div>
            <label className="block text-xs text-gray-500 mb-1.5 font-medium">Квота данных (байт)</label>
            <input className="input" type="number" placeholder="не ограничено" value={form.data_quota_bytes} onChange={e => set('data_quota_bytes', e.target.value)} />
          </div>
          <div>
            <label className="block text-xs text-gray-500 mb-1.5 font-medium">Срок истечения (RFC3339)</label>
            <input className="input" placeholder="2025-12-31T23:59:59Z" value={form.expiration_rfc3339} onChange={e => set('expiration_rfc3339', e.target.value)} />
          </div>
          <div>
            <label className="block text-xs text-gray-500 mb-1.5 font-medium">Лимит загрузки (бит/с)</label>
            <input className="input" type="number" placeholder="не ограничено" value={form.rate_limit_up_bps} onChange={e => set('rate_limit_up_bps', e.target.value)} />
          </div>
          <div>
            <label className="block text-xs text-gray-500 mb-1.5 font-medium">Лимит скачивания (бит/с)</label>
            <input className="input" type="number" placeholder="не ограничено" value={form.rate_limit_down_bps} onChange={e => set('rate_limit_down_bps', e.target.value)} />
          </div>
          <div>
            <label className="block text-xs text-gray-500 mb-1.5 font-medium">AD-тег (32 hex)</label>
            <input className="input" placeholder="опционально" value={form.user_ad_tag} onChange={e => set('user_ad_tag', e.target.value)} />
          </div>
...
function EditUserModal({ user, userDetail, onClose, onUpdated }) {
  const [form, setForm] = useState({
    max_tcp_conns: userDetail?.max_tcp_conns ?? '',
    max_unique_ips: userDetail?.max_unique_ips ?? '',
    data_quota_bytes: userDetail?.data_quota_bytes ?? '',
    rate_limit_up_bps: userDetail?.rate_limit_up_bps ?? '',
    rate_limit_down_bps: userDetail?.rate_limit_down_bps ?? '',
    expiration_rfc3339: userDetail?.expiration_rfc3339 ?? '',
    user_ad_tag: userDetail?.user_ad_tag ?? '',
  })
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState(null)
  const toast = useToast()
  ...
  const submit = async () => {
    ...
    try {
      const body = {}
      if (form.max_tcp_conns !== '') body.max_tcp_conns = parseInt(form.max_tcp_conns) || null
      if (form.max_unique_ips !== '') body.max_unique_ips = parseInt(form.max_unique_ips) || null
      if (form.data_quota_bytes !== '') body.data_quota_bytes = parseInt(form.data_quota_bytes) || null
      if (form.rate_limit_up_bps !== '') body.rate_limit_up_bps = parseInt(form.rate_limit_up_bps) || null
      if (form.rate_limit_down_bps !== '') body.rate_limit_down_bps = parseInt(form.rate_limit_down_bps) || null
      if (form.expiration_rfc3339 !== '') body.expiration_rfc3339 = form.expiration_rfc3339 || null
      if (form.user_ad_tag !== '') body.user_ad_tag = form.user_ad_tag || null

      await api.patchUser(user.username, body)
  ...
  return (
    <Modal title={`Редактировать: ${user.username}`} onClose={onClose} size="lg">
      <div className="space-y-4">
        {error && (
          <div className="flex items-center gap-2 p-3 bg-red-900/20 border border-red-700/40 rounded-lg text-sm text-red-300">
            <AlertTriangle size={14} /> {error}
          ...
          {(user.links?.tls?.length > 0 || user.links?.classic?.length > 0 || user.links?.secure?.length > 0) && (
            <div>
              ...
              <div className="space-y-2">
                {[
                  ['TLS', user.links?.tls],
                  ['Secure', user.links?.secure],
                  ['Classic', user.links?.classic],
                ].map(([type, links]) =>
                  links?.map((link, i) => (
                    <div key={`${type}-${i}`} className="flex items-center gap-2">
                      <span className="text-xs text-gray-500 w-12">{type}</span>
                      <code className="flex-1 text-xs font-mono bg-dark-700 px-2 py-1.5 rounded text-blue-300 break-all overflow-hidden">{link}</code>
                      <button
                        onClick={() => copy(link, `${type}-${i}`)}
...
function RotateSecretModal({ username, onClose, onDone }) {
  ...
  if (result) {
    return (
      <Modal title="Секрет обновлён" onClose={onClose}>
        <div className="space-y-4">
          <div className="p-4 bg-green-900/20 border border-green-700/40 rounded-xl">
            <div className="text-sm font-medium text-green-300 mb-3">Новый секрет пользователя <b>{username}</b>:</div>
            <div className="flex items-center gap-2">
              <code className="flex-1 font-mono text-sm bg-dark-900 px-3 py-2 rounded-lg text-yellow-300 border border-dark-600 break-all">
                {result.secret || result.new_secret || JSON.stringify(result)}
              </code>
              <CopyButton text={result.secret || result.new_secret || ''} />
            </div>
          </div>
          <button onClick={onClose} className="btn-primary w-full justify-center">Закрыть</button>
        </div>
    ...
    <Modal title={`Сменить секрет: ${username}`} onClose={onClose}>
      <div className="space-y-4">
        ...
        <div className="flex gap-3">
          <button onClick={onClose} className="btn-ghost flex-1 justify-center">Отмена</button>
          <button onClick={submit} disabled={loading} className="btn-warning flex-1 justify-center">
...
export default function Users() {
  ...
  const handleAction = async (action, user) => {
    if (action === 'view') {
      setModal({ type: 'view', username: user.username })
    } else if (action === 'rotate') {
      setModal({ type: 'rotate', username: user.username })
      ...
      setModal({
        ...
        onConfirm: async () => {
          try {
            ...
            toast(`Пользователь "${user.username}" удалён`, 'success')
            setModal(null)
            load()
          } catch (e) {
      ...
      setModal({
        ...
        message: `Отключить "${user.username}"? Пользователь не сможет подключаться.`,
        danger: false,
        onConfirm: async () => {
          try {
            await api.disableUser(user.username)
            toast(`Пользователь "${user.username}" отключён`, 'success')
            setModal(null)
            load()
          } catch (e) {
      ...
      try {
        ...
        toast(`Пользователь "${user.username}" активирован`, 'success')
        load()
      } catch (e) {
        toast('Ошибка: ' + e.message, 'error')
      }
      ...
      setModal({
        ...
        message: `Сбросить счётчик использованных данных для "${user.username}"?`,
        danger: false,
        onConfirm: async () => {
          try {
            await api.resetQuota(user.username)
            toast(`Квота "${user.username}" сброшена`, 'success')
            setModal(null)
            load()
          } catch (e) {
            toast('Ошибка: ' + e.message, 'error')
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">{users.length} пользователей настроено</p>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="card p-0 overflow-hidden">
        {loading ? (
          <div className="p-6 space-y-3">
            {[1,2,3,4].map(i => <div key={i} className="h-12 bg-dark-700 animate-pulse rounded-lg" />)}
          </div>
        ) : filtered.length === 0 ? (
          <div className="text-center py-16 text-gray-600">
            <UsersIcon size={40} className="mx-auto mb-3 opacity-30" />
            <div className="text-sm">
              {users.length === 0 ? 'Нет пользователей. Создайте первого!' : 'Ничего не найдено'}
            </div>
          </div>
        ) : (
                  ...
                  <tr key={user.username} className="table-row-hover">
                    <td className="px-4 py-3">
                      ...
                      {user.expiration_rfc3339 && (
                        <div className="text-xs text-gray-500 mt-0.5">до {user.expiration_rfc3339?.slice(0, 10)}</div>
                      )}
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        {user.enabled !== false
                          ? <span className="badge-green">Активен</span>
                          : <span className="badge-red">Откл.</span>
                        }
                        {!user.in_runtime && (
                          <span className="badge-yellow">вне runtime</span>
                        )}
                      </div>
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-300">{user.current_connections ?? 0}</td>
                    <td className="px-4 py-3 text-sm text-gray-300">
                      {user.active_unique_ips ?? 0}
                      {user.max_unique_ips ? `/${user.max_unique_ips}` : ''}
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-300">{formatBytes(user.total_octets)}</td>
                    <td className="px-4 py-3 text-sm text-gray-300">
                      {user.data_quota_bytes ? formatBytes(user.data_quota_bytes) : '∞'}
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-300">
                      {user.rate_limit_up_bps || user.rate_limit_down_bps
                        ? `${formatBps(user.rate_limit_up_bps)} / ${formatBps(user.rate_limit_down_bps)}`
                        : '∞'}
                    </td>
                    <td className="px-4 py-3">
      ...
      {modal?.type === 'create' && (
        <CreateUserModal onClose={() => setModal(null)} onCreated={load} />
      )}
      {modal?.type === 'view' && (
        <UserDetailModal username={modal.username} onClose={() => setModal(null)} />
      )}
      {modal?.type === 'rotate' && (
        <RotateSecretModal username={modal.username} onClose={() => setModal(null)} onDone={load} />
      )}
      {modal?.type === 'confirm' && (
        <ConfirmModal
          title={modal.title}
