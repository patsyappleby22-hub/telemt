...
import { Shield, RefreshCw, CheckCircle, XCircle, Lock, Wifi, List, Server } from 'lucide-react'
...
function BoolRow({ label, value }) {
  return (
    <div className="flex items-center justify-between py-2.5 border-b border-dark-700 last:border-0">
      ...
      {value ? <span className="badge-green"><CheckCircle size={10} />Включено</span> : <span className="badge-red"><XCircle size={10} />Выключено</span>}
    </div>
  )
}
...
export default function Security() {
  ...
  const load = useCallback(async (showRefresh = false) => {
    ...
    try {
      const [p, w, l] = await Promise.allSettled([api.securityPosture(), api.securityWhitelist(), api.limitsEffective()])
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="space-y-4">{[1,2,3].map(i => <div key={i} className="card h-40 animate-pulse bg-dark-700" />)}</div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {posture && (
            <div className="card">
              <h2 className="text-sm font-semibold text-gray-300 mb-4 flex items-center gap-2"><Lock size={15} className="text-blue-400" />Уровень защиты</h2>
              <div>
                {Object.entries(posture).map(([k, v]) => {
                  if (typeof v === 'boolean') return <BoolRow key={k} label={k.replace(/_/g, ' ')} value={v} />
                  if (typeof v === 'string' || typeof v === 'number') return <ValueRow key={k} label={k.replace(/_/g, ' ')} value={String(v)} />
                  return null
                })}
              </div>
            </div>
          ...
          {whitelist && (
            <div className="card">
              <h2 className="text-sm font-semibold text-gray-300 mb-4 flex items-center gap-2"><List size={15} className="text-green-400" />Белые списки API</h2>
                ...
                <div key={section} className="mb-4 last:mb-0">
                  ...
                  {Array.isArray(entries) && entries.length > 0 ? (
                    <div className="space-y-1">
                      {entries.map((e, i) => <div key={i} className="px-3 py-1.5 bg-dark-700 rounded text-xs font-mono text-gray-300">{e}</div>)}
                    </div>
          ...
          {limits && (
            <div className="card lg:col-span-2">
              <h2 className="text-sm font-semibold text-gray-300 mb-4 flex items-center gap-2"><Wifi size={15} className="text-yellow-400" />Лимиты системы</h2>
