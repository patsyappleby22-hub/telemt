...
async function editScreen(bot, query, text, opts = {}) {
  ...
  if (r?.ok) { lastMsg.set(userId, { chatId, msgId }); return }
...
async function startBot() {
  ...
  if (!token) {
    console.log('[bot] Токен не настроен. Добавьте BOT_TOKEN в переменные окружения.')
